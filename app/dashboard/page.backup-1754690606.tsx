...
export default async function Dashboard() {
  ...
  // --- Latest News (Trades + Commissioner with real details) ---
  ...
  const commishDetail = (t:any) => {
    const note = (t.note || t.comments || t.message || "").trim();
    if (note) return note;

    const players = Array.isArray(t.players) ? t.players : [];
    const bits = players.map((p:any) => {
      const pd = p?.transaction_data || p?.player_transaction_data || {};
      const move = (pd.type || pd.sub_type || "").replace(/_/g, " ");
      ...
      if (move && (src || dst)) {
        const where = dst ? ` to ${dst}` : src ? ` from ${src}` : "";
        return `${move}: ${name}${where}`;
      }
      if (move) return `${move}: ${name}`;
      return name ? `Action on ${name}` : "";
    }).filter(Boolean);

    return bits.join("; ") || "Commissioner action.";
  };
  ...
  const newsItems = txs
    ...
    .map((t:any) => {
      ...
      return {
        id: t.transaction_key || when,
        title: type === "trade" ? "Trade" : "Commissioner",
        when,
        rel: when ? relTime(when) : "",
        detail,
      };
    });
