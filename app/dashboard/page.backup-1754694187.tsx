...
export default async function Dashboard() {
  ...
  const gm = await yf.game.meta("nfl").catch(()=>null);
  const candidates:any[] = [];
  if (gm) {
    if (Array.isArray(gm)) candidates.push(...(gm as any).map((g:any)=>g?.game_key));
    candidates.push((gm as any)?.game_key, (gm as any)?.[0]?.game_key);
  }
  candidates.push(process.env.YAHOO_GAME_KEY, "461"); // fallback to NFL 2025
  const gameKey = candidates.find((x:any) => x && String(x).trim());
  const leagueKey = `${gameKey}.l.${process.env.YAHOO_LEAGUE_ID}`;
  console.log("[DBG] gameKey=", gameKey, "leagueKey=", leagueKey);

  const meta = await yf.league.meta(leagueKey).catch(() => ({}));
  const week = Number(meta?.current_week || 1);

  const rawSettings:any = await yf.league.settings(leagueKey).catch(()=>null);
  const st:any = (rawSettings?.settings ?? rawSettings?.league?.settings ?? rawSettings) || {};
  const pick = (...xs:any[]) => {
    for (const v of xs) {
  ...
  const draftTime = pick(st.draft_time, st.draft_timestamp, st.draft_date, st?.draft?.time, st?.draft?.date, meta?.draft_time, meta?.draft_timestamp, meta?.draft_date);
  const draftType = pick(st.draft_type, st?.draft?.type, (st.auction_draft ? "auction" : ""), (st.snake_draft ? "snake" : ""));
  const tradeDL   = pick(meta?.trade_end_date, st.trade_end_date, st.trade_deadline);
  const waiver    = pick(st.waiver_type, st.waiver_rule);
  const rosterL   = pick(st.roster_locktime, st.roster_locked, st.roster_change_deadline);
  ...
  const standings = (teams || [])
    .map((t: any) => {
      const stg = t.standings || t.team_standings || {};
      const ot = stg.outcome_totals || t.outcome_totals || {};
      return { name: t.name, owner: t.managers?.[0]?.nickname || "Owner", w: +(ot.wins || 0), l: +(ot.losses || 0), pf: +(stg.points_for || 0) };
    })
  ...
  else if (meta?.draft_status) cuLines.push(`Draft status: ${meta.draft_status}`);
  if (tradeDL) cuLines.push(`Trade deadline: ${fmtDate(tradeDL)}`);
  if (waiver) cuLines.push(`Waivers: ${String(waiver).replace(/_/g," ")}`);
  if (rosterL) cuLines.push(`Roster lock: ${fmtDate(rosterL)}`);
